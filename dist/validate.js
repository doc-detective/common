...
export function validate({ schemaKey, object, addDefaults = true, }) {
    ...
    if (check.errors) {
        ...
        const matchedSchemaKey = compatibleSchemasList.find((key) => {
            ...
            if (check && check(validationObject))
                return key;
        });
        if (!matchedSchemaKey) {
            result.errors = check.errors
...
export function transformToSchemaKey({ currentSchema = "", targetSchema = "", object = {}, }) {
    ...
    if (targetSchema === "step_v3") {
        ...
        if (object?.runTests?.contexts)
            transformedObject.runOn = object.runTests.contexts.map((context) => transformToSchemaKey({
        ...
        if (object?.integrations?.openApi) {
            ...
            transformedObject.integrations.openApi = object.integrations.openApi.map((description) => transformToSchemaKey({
                currentSchema: "openApi_v2",
                targetSchema: "openApi_v3",
                object: description,
            }));
            ...
            transformedObject.fileTypes = object.fileTypes.map((fileType) => {
                const transformedFileType = {
                    ...
                    inlineStatements: {
                        ...
                        testStart: `${escapeRegExp(fileType.testStartStatementOpen)}(.*?)${escapeRegExp(fileType.testStartStatementClose)}`,
                        testEnd: escapeRegExp(fileType.testEndStatement),
                        ignoreStart: escapeRegExp(fileType.testIgnoreStatement),
                        step: `${escapeRegExp(fileType.stepStatementOpen)}(.*?)${escapeRegExp(fileType.stepStatementClose)}`,
                    },
                };
                if (fileType.markup)
        ...
        if (object.contexts)
            transformedObject.runOn = object.contexts.map((context) => transformToSchemaKey({
                currentSchema: "context_v2",
                targetSchema: "context_v3",
                object: context,
            }));
        if (object.openApi)
            transformedObject.openApi = object.openApi.map((description) => transformToSchemaKey({
                currentSchema: "openApi_v2",
                targetSchema: "openApi_v3",
                object: description,
            }));
        transformedObject.tests = object.tests.map((test) => transformToSchemaKey({
        ...
        transformedObject.steps = object.steps.map((step) => transformToSchemaKey({

 ...
 */
export function getNodeTextContent(node: AstNode): string {
 ...
 */
export function nodeMatches(node: AstNode, config: AstMatchConfig): boolean {
  // Check nodeType
  if (config.nodeType !== undefined) {
    if (typeof config.nodeType === "string") {
      if (node.type !== config.nodeType) return false;
    } else if (Array.isArray(config.nodeType)) {
      if (!config.nodeType.includes(node.type)) return false;
    }
  }
  ...
  if (config.content !== undefined) {
    ...
    if (!matchesPattern(text, config.content)) return false;
  }
 ...
 */
function getByPath(node: AstNode, path: string): any {
